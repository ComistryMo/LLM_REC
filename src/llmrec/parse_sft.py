...
from .utils import read_records, write_jsonl
...
def convert_record(record: dict[str, Any], idx: int) -> dict[str, Any]:
    ...
    record_id = record.get("id") or record.get("sample_id") or f"sft-{idx:08d}-{uuid.uuid4().hex[:8]}"
    task_type = record.get("task_type") or infer_task_type(prompt, output)
    ...
    return {
        ...
        "input_len": token_len(prompt),
        "output_len": token_len(output),
        "itemic_count": len(itemic_tokens),
        "source": str(record.get("source") or record.get("dataset") or "sft"),
    }


def parse_sft_files(inputs: list[str | Path], output: str | Path) -> int:
    ...
    return write_jsonl(converted, output)
